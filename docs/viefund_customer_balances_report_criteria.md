...
Do not select `Deleted` to reproduce the historical report. Cash transaction `4733867` for `6106341PPR` is currently deleted, while its linked fund and trust rows have `dtLastModified` values of 1:54 PM Eastern on July 30, after the report had already been emailed. Including it reproduces the validated historical report. The linked timestamps show that related records changed later, but do not identify the changed fields or independently prove when the cash status became Deleted.

When a simulated generation time is supplied, the report applies this inference narrowly: a currently deleted cash row is included only when `UB_FundTrx.dtLastModified` or `UB_TrustTrx.dtLastModified` is later than the simulated generation time. `UB_FundTrx` is linked through `UB_FundTrxCash`; `UB_TrustTrx` is linked through `UB_CashTrx.iTrustTrxID`. Transactions created after the simulated time are excluded. Without a simulated generation time, current statuses are used as-is.
...
The export summary separates future-settling cash into `FSC Linked to Used Trust (Client Reported)` and `FSC Linked to Unused Trust`. These values are the full cash amounts classified by linked trust usage, not the numeric `mAmountUsed` values themselves. On a Direct Cash Ledger settlement-date report, cash is classified as linked to used trust when its linked Unsettled trust has `mAmountUsed > 0` or `mAmountLeft <= 0`; it is classified as linked to unused trust when `mAmountUsed = 0` and `mAmountLeft > 0`. Thus `Future Settlement Cash (Info)` equals the used-linked plus unused-linked cash, and `Potential Client VieFund Balance (Estimate)` equals the settled report total plus the used-linked cash. On trade-date and other report modes, no separate future amount is added; all informational future cash is classified as unused-linked for the estimate. Eligible trade-date cash is already represented by the selected date basis. The estimate is derived from current ledger and linked trust state and is not a stored client-report snapshot.
