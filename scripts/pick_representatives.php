...
foreach ($grouped as $dealer => $groupRows) {
    ...
    if ($examples['historical_ended'] === null && $hasEndDate) {
        $examples['historical_ended'] = ['dealer' => $dealer, 'rows' => $groupRows];
    }
    if ($examples['active_no_cash'] === null && $hasNoCash) {
        $examples['active_no_cash'] = ['dealer' => $dealer, 'rows' => $groupRows];
    }
    if ($examples['legacy_alias'] === null && $hasLegacyAlias) {
        $examples['legacy_alias'] = ['dealer' => $dealer, 'rows' => $groupRows];
    }
}
