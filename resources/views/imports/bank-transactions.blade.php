...
@section('content')
    ...
    <div class="card" style="margin-bottom: 20px;">
        <form action="{{ route('imports.bank.transactions') }}" method="GET" style="display: flex; gap: 10px;">
            ...
            <div style="display: flex; gap: 10px;">
                ...
                @if(request('search'))
                    <a href="{{ route('imports.bank.transactions') }}" class="btn" style="background: #718096; padding: 10px 20px; text-decoration: none;">Clear</a>
                @endif
            </div>
        </form>
    </div>
    ...
    <div class="card">
                    ...
                    <tbody>
                        @foreach($transactions as $transaction)
                            <tr class="bank-row" style="border-bottom: 1px solid #e2e8f0; cursor: pointer;"
                                data-txn-date="{{ $transaction->txn_date }}"
        ...
        @else
            <div style="text-align: center; padding: 60px 40px; color: #718096;">
                ...
                <a href="{{ route('imports.index') }}" class="btn" style="background: var(--primary-blue); padding: 10px 30px; text-decoration: none; color: white; display: inline-block;">
                    Go to Import Page
                </a>
            </div>
        @endif
            ...
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <div>
                    ...
                    <p style="color: #742a2a; font-size: 14px; margin: 0;">
                        This will permanently delete all {{ number_format($totalRecords) }} bank transaction records from the database.
                    </p>
                </div>
                <form action="{{ route('imports.bank.truncate') }}" method="POST" style="margin: 0;"
