            ...
            <table style="width:100%;border-collapse:collapse;" class="mono-grid">
                <thead>
                    <tr style="background:#f7fafc;border-bottom:2px solid #e2e8f0;white-space:nowrap;">
                        ...
                        <th style="text-align:right;font-weight:600;color:#2d3748;line-height:1.25;"><a href="{{ $sortUrl('settlement_net_total') }}" style="color:inherit;text-decoration:none;"><span style="display:block;">Bank EFT</span><span style="display:block;">Net{{ $sortArrow('settlement_net_total') }}</span></a></th>
                ...
                <tbody>
                    @foreach($rows as $row)
                        <tr style="border-bottom:1px solid #e2e8f0;background:{{ abs($row['variance']) >= .01 ? '#fff5f5' : ($loop->even ? 'rgba(56,161,105,.07)' : 'transparent') }};">
                            ...
                            <td style="text-align:right;color:#4a5568;" title="{{ $row['settlement_sequences']->implode(', ') }}">@if($row['settlement_transaction_count'] > 0)<a href="{{ route('reconciliations.daily-totals.bank-day', ['date' => $row['total_date'], 'settlement_numbers' => $row['settlement_sequences']->implode(','), 'all_dates' => 1]) }}" target="_blank" rel="noopener noreferrer" style="color:inherit;text-decoration:underline;">{{ number_format($row['settlement_sequences']->count()) }}</a>@else 0 @endif</td>
                            <td style="text-align:right;font-weight:500;color:{{ $row['settlement_net_total'] < 0 ? '#e53e3e' : '#276749' }};white-space:nowrap;">{{ $formatMoney($row['settlement_transaction_count'] > 0 ? $row['settlement_net_total'] : 0) }}</td>
                            <td style="text-align:right;color:#4a5568;" title="Matched {{ $row['matched_sequence_count'] }} of {{ $row['settlement_sequences']->count() }} distinct sequence(s)">@if($row['settlement_transaction_count'] > 0)<a href="{{ route('eft-files.index', ['tab' => 'items', 'sequences' => $row['matched_sequences']->implode(','), 'drilldown_date' => $row['total_date']]) }}" target="_blank" rel="noopener noreferrer" style="color:inherit;text-decoration:underline;">{{ number_format($row['eft_transaction_count']) }}</a>@else 0 @endif</td>
                            <td style="text-align:right;font-weight:500;color:{{ $row['eft_net_total'] < 0 ? '#e53e3e' : '#276749' }};white-space:nowrap;">{{ $formatMoney($row['settlement_transaction_count'] > 0 ? $row['eft_net_total'] : 0) }}</td>
                            <td style="text-align:right;font-weight:600;color:{{ abs($row['variance']) >= .01 ? '#e53e3e' : '#276749' }};white-space:nowrap;"><a href="{{ route('reconciliations.daily-totals.settlement-sequences', ['date' => $row['total_date'], 'account' => $row['account_number'], 'include_3000_sequences' => $include3000Sequences ? 1 : 0]) }}" target="_blank" rel="noopener noreferrer" style="color:inherit;text-decoration:underline;">{{ $formatMoney($row['variance']) }}</a></td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;margin-top:14px;flex-wrap:wrap;">
                <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
                    ...
                    <form method="GET" action="{{ route('reconciliations.daily-totals') }}" style="display:flex;align-items:center;gap:6px;margin:0;">
                        @foreach(request()->except(['per_page', 'page', '_results']) as $key => $value)
                        ...
                        <select id="per_page" name="per_page" data-async-results-select style="padding:5px 8px;border:1px solid #cbd5e0;border-radius:4px;font-size:12px;">
                            @foreach([25, 50, 100, 250] as $option)<option value="{{ $option }}" @selected($perPage === $option)>{{ $option }}</option>@endforeach
                        </select>
                    </form>
                </div>
