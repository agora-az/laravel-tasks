                ...
                <tbody>
                    @foreach($rows as $row)
                        ...
                        <tr style="border-bottom:1px solid #e2e8f0;background:{{ !$isMatch ? 'rgba(229,62,62,.08)' : ($loop->even ? 'rgba(56,161,105,.07)' : 'transparent') }};">
                            ...
                            <td style="text-align:right;color:#4a5568;">@if($row['eft_transaction_count'] > 0)<a href="{{ route('eft-files.index', ['tab' => 'items', 'sequences' => $row['sequence']]) }}" target="_blank" rel="noopener noreferrer" style="color:inherit;text-decoration:underline;">{{ number_format($row['eft_transaction_count']) }}</a>@else 0 @endif</td>
