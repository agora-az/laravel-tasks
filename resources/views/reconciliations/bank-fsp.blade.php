...
@php
    ...
    $sortUrl = fn(string $field) => route('reconciliations.bank-fsp.source', array_merge(['source'=>$source], request()->except(['sort','sort_dir','page']), ['sort'=>$field,'sort_dir'=>$sortField === $field && $sortDir === 'asc' ? 'desc' : 'asc']));
...
<div class="card" style="margin-bottom:20px;">
    <form method="GET" action="{{ route('reconciliations.bank-fsp.source', ['source'=>$source]) }}" style="display:grid;grid-template-columns:1fr 1fr auto;gap:12px;align-items:end;">
        <div>
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;margin-top:14px;flex-wrap:wrap;">
                <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
                    ...
                    <form method="GET" action="{{ route('reconciliations.bank-fsp.source', ['source'=>$source]) }}" style="display:flex;align-items:center;gap:6px;margin:0;">
                        @foreach(request()->except(['per_page','page']) as $key => $value) @if(!is_array($value))<input type="hidden" name="{{ $key }}" value="{{ $value }}">@endif @endforeach
                        <label for="fsp_per_page" style="font-size:12px;color:#718096;">Rows</label>
                        <select id="fsp_per_page" name="per_page" onchange="this.form.submit()" style="padding:5px 8px;border:1px solid #cbd5e0;border-radius:4px;font-size:12px;">@foreach([25,50,100,250] as $option)<option value="{{ $option }}" @selected($perPage === $option)>{{ $option }}</option>@endforeach</select>
                    </form>
                </div>
